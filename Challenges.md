...
Improve `my_free` so we loop through the blocks from the start and glue blocks together if they are free consecutively (we call this coalescing).

### Part B: Efficient Coalescing
Rewrite our system so that when we `my_free(ptr)` a pointer `ptr`, it checks the block before and after it to see if we could coalesce them together.
